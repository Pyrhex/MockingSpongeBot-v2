...
class Gro(commands.Cog):
    ...
    gro = SlashCommandGroup("gro", description="Grocery list management", guild_ids=[1065813802425262141])
...
#                 await ctx.respond("You have no grocery lists.")
    @gro.command()
    async def delete(self, ctx, listname):
            if listname == "all":
                groceries = {}
                await ctx.respond("Deleted all grocery lists.")
                jsonfile = json.dumps(groceries, indent = 4)
                ...
                if listname not in list(groceries.keys()):
                    ...
                    groceries.pop(listname)
                    await ctx.respond(f"Deleted **{listname}**.")
                    jsonfile = json.dumps(groceries, indent = 4)
                    with open(savefile, "w") as f:
                        f.write(jsonfile)
    @gro.command()
    async def clear(self, ctx, listname):
        ...
        if listname == "all":
            ...
            with open(savefile, "w") as f:
                f.write(jsonfile)
            await ctx.respond(f"Cleared all lists.")
        elif listname not in list(groceries.keys()):
            if listname != "":
                await ctx.respond(f"Unrecognized list: **{listname}**")
            else:
                await ctx.respond(f"Missing list argument.")
            ...
            groceries[listname] = []
            await ctx.respond(f"Cleared **{listname}**.")
            jsonfile = json.dumps(groceries, indent = 4)
            with open(savefile, "w") as f:
                f.write(jsonfile)
    @gro.command()
    async def new(self, ctx, listname):
        ...
        with open(savefile, "w") as f:
            f.write(jsonfile)
        await ctx.respond(f"Created new list: **{listname}**")
    ...
    async def add(self, ctx, listname, item):
        ...
        if item != "":
            bulk = item.replace(", ", ",").split(",")
            for i in bulk:
                groceries[listname].append(i)
            await ctx.respond(f"Added *{item}* to **{listname}**.")
